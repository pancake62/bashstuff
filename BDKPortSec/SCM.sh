...
set timeout 60
# Set variables
 set hostname "10.9.1.7"
 set username "jjamal"
 set password "Julianna123!@#"
 set enablepassword "Julianna123!@#"

# Log results
 log_file -a ~/Scripts/Logs/psec_DSSA2960XSCM.txt
...
# Let's go to configure mode
 ...
 expect "(config-if-range)#"
 send "authentication event fail action authorize vlan 7\r"
 expect "(config-if-range)#"
 send "authentication event server dead action authorize vlan 7\r"
 expect "(config-if-range)#"
