...
set timeout 60
# Set variables
 set hostname "10.9.1.9"
 set username "jjamal"
 set password "Julianna123!@#"
 set enablepassword "Julianna123!@#"

# Log results
 log_file -a ~/Scripts/Logs/psec_PUMPRM.txt
...
# Let's go to configure mode
 send "conf t\n"
 expect "(config)#"
 send "int range gi1/0/1-6\r"
 expect "(config-if-range)#"
 send "switchport mode access\r"
 expect "(config-if-range)#"
 send "switchport port-security aging time 2\r"
 expect "(config-if-range)#"
 send "authentication event fail action authorize vlan 133\r"
 expect "(config-if-range)#"
 send "authentication event server dead action authorize vlan 133\r"
 expect "(config-if-range)#"
